...
while True:
    ...
    if (correction == 0):
        msg = f"Data received with no error : {data}"
        k.send(msg.encode())
    else:
        data1 = ''
        ...
        msg = f"Data received is {data}.\nError found at position {m-correction}.\nCorrected data is {data1}"
        k.send(msg.encode())
    ...
    if (correction == 0):
        msg = f"Data received without error : {ndata}"
        k.send(msg.encode())
    else:
        data1 = ''
